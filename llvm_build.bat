...
set SOC_DRIVER_LED=%PROJECT_PATH%\driver\led
...
for %%x in (%SOC_DRIVER_LED%\*.c) do (
%COMPILER%  -c  %DAM_CPPFLAGS% %GAGENT_CPPFLAGS% %DAM_CFLAGS% %APP_CFLAGS% %DAM_INCPATHS% %GAGENT_INC% %SOC_DRIVER_LED% %%x 
 if errorlevel 1 (goto :eof)
)
